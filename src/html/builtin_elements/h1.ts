...
export type Heading1Props<T = unknown> = IncomingProps<HTMLHeadingElement, T>;

export class Heading1Element<T = unknown> extends HTMLHeadingElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'h1' });
    }
  }
  ...
  static getSelector() {
    return `h1[is="${this.getName()}"]`;
  }

  props: Partial<Heading1Props<T>>;

  constructor(props?: Partial<Heading1Props<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<Heading1Props>} Partial props.
   */
  getDefaultProps(): Partial<Heading1Props<T>> {
    return {};
  }
...
// Shorthand alias
export { Heading1Element as Heading1 };
