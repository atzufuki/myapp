...
export type HtmlProps<T = unknown> = IncomingProps<HTMLHtmlElement, T>;

export class HtmlElement<T = unknown> extends HTMLHtmlElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'html' });
    }
  }
  ...
  static getSelector() {
    return `html[is="${this.getName()}"]`;
  }

  props: Partial<HtmlProps<T>>;

  constructor(props?: Partial<HtmlProps<T>>) {
    super();
    this.props = props ?? {};
  }
