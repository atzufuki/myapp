...
export type SpanProps<T = unknown> = IncomingProps<HTMLSpanElement, T>;

export class SpanElement<T = unknown> extends HTMLSpanElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'span' });
    }
  }
  ...
  static getSelector() {
    return `span[is="${this.getName()}"]`;
  }

  props: Partial<SpanProps<T>>;

  constructor(props?: Partial<SpanProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<SpanProps>} Partial props.
   */
  getDefaultProps(): Partial<SpanProps<T>> {
    return {};
  }
