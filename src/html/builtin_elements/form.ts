...
export type FormProps<T = unknown> = IncomingProps<HTMLFormElement, T>;

export class FormElement<T = unknown> extends HTMLFormElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'form' });
    }
  }
  ...
  static getSelector() {
    return `form[is="${this.getName()}"]`;
  }

  props: Partial<FormProps<T>>;

  constructor(props?: Partial<FormProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<FormProps>} Partial props.
   */
  getDefaultProps(): Partial<FormProps<T>> {
    return {};
  }
