...
export type ButtonProps<T = unknown> = IncomingProps<HTMLButtonElement, T>;

export class ButtonElement<T = unknown> extends HTMLButtonElement {
  ...
  static getSelector() {
    return `button[is="${this.getName()}"]`;
  }

  props: Partial<ButtonProps<T>>;

  constructor(props?: Partial<ButtonProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<ButtonProps>} Partial props.
   */
  getDefaultProps(): Partial<ButtonProps<T>> {
    return {};
  }
