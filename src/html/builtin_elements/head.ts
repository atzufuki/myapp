...
export type HeadProps<T = unknown> = IncomingProps<HTMLHeadElement, T>;

export class HeadElement<T = unknown> extends HTMLHeadElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'head' });
    }
  }
  ...
  static getSelector() {
    return `head[is="${this.getName()}"]`;
  }

  props: Partial<HeadProps<T>>;

  constructor(props?: Partial<HeadProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<HeadProps>} Partial props.
   */
  getDefaultProps(): Partial<HeadProps<T>> {
    return {};
  }
