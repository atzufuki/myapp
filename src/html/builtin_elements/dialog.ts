...
export type DialogProps<T = unknown> = IncomingProps<HTMLDialogElement, T>;

export class DialogElement<T = unknown> extends HTMLDialogElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'dialog' });
    }
  }
  ...
  static getSelector() {
    return `dialog[is="${this.getName()}"]`;
  }

  props: Partial<DialogProps<T>>;

  constructor(props?: Partial<DialogProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<DialogProps>} Partial props.
   */
  getDefaultProps(): Partial<DialogProps<T>> {
    return {};
  }
...
// Shorthand alias
export { DialogElement as Dialog };
