...
export type BoldProps<T = unknown> = IncomingProps<HTMLElement, T>;

export class BoldElement<T = unknown> extends HTMLElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'b' });
    }
  }
  ...
  static getSelector() {
    return `b[is="${this.getName()}"]`;
  }

  props: Partial<BoldProps<T>>;

  constructor(props?: Partial<BoldProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<BoldProps>} Partial props.
   */
  getDefaultProps(): Partial<BoldProps<T>> {
    return {};
  }
