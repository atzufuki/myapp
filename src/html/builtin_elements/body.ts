...
export type BodyProps<T = unknown> = IncomingProps<HTMLBodyElement, T>;

export class BodyElement<T = unknown> extends HTMLBodyElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'body' });
    }
  }
  ...
  static getSelector() {
    return `body[is="${this.getName()}"]`;
  }

  props: Partial<BodyProps<T>>;

  constructor(props?: Partial<BodyProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<BodyProps>} Partial props.
   */
  getDefaultProps(): Partial<BodyProps<T>> {
    return {};
  }
