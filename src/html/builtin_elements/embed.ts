...
export type EmbedProps<T = unknown> = IncomingProps<HTMLEmbedElement, T>;

export class EmbedElement<T = unknown> extends HTMLEmbedElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'embed' });
    }
  }
  ...
  static getSelector() {
    return `embed[is="${this.getName()}"]`;
  }

  props: Partial<EmbedProps<T>>;

  constructor(props?: Partial<EmbedProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<EmbedProps>} Partial props.
   */
  getDefaultProps(): Partial<EmbedProps<T>> {
    return {};
  }
