...
export type ScriptProps<T = unknown> = IncomingProps<HTMLScriptElement, T>;

export class ScriptElement<T = unknown> extends HTMLScriptElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'script' });
    }
  }
  ...
  static getSelector() {
    return `script[is="${this.getName()}"]`;
  }

  props: Partial<ScriptProps<T>>;

  constructor(props?: Partial<ScriptProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<ScriptProps>} Partial props.
   */
  getDefaultProps(): Partial<ScriptProps<T>> {
    return {};
  }
...
// Shorthand alias
export { ScriptElement as Script };
