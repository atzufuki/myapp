...
export type TextAreaProps<T = unknown> = IncomingProps<HTMLTextAreaElement, T>;

export class TextAreaElement<T = unknown> extends HTMLTextAreaElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'textarea' });
    }
  }
  ...
  static getSelector() {
    return `textarea[is="${this.getName()}"]`;
  }

  props: Partial<TextAreaProps<T>>;

  constructor(props?: Partial<TextAreaProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TextAreaProps>} Partial props.
   */
  getDefaultProps(): Partial<TextAreaProps<T>> {
    return {};
  }
...
// Shorthand alias
export { TextAreaElement as TextArea };
