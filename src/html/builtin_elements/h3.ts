...
export type Heading3Props<T = unknown> = IncomingProps<HTMLHeadingElement, T>;

export class Heading3Element<T = unknown> extends HTMLHeadingElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'h3' });
    }
  }
  ...
  static getSelector() {
    return `h3[is="${this.getName()}"]`;
  }

  props: Partial<Heading3Props<T>>;

  constructor(props?: Partial<Heading3Props<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<Heading3Props>} Partial props.
   */
  getDefaultProps(): Partial<Heading3Props<T>> {
    return {};
  }
...
// Shorthand alias
export { Heading3Element as Heading3 };
