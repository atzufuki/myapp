...
export type DivProps<T = unknown> = IncomingProps<HTMLDivElement, T>;

export class DivElement<T = unknown> extends HTMLDivElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'div' });
    }
  }
  ...
  static getSelector() {
    return `div[is="${this.getName()}"]`;
  }

  props: Partial<DivProps<T>>;

  constructor(props?: Partial<DivProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<DivProps>} Partial props.
   */
  getDefaultProps(): Partial<DivProps<T>> {
    return {};
  }
