...
export class OptionElement<T = unknown> extends HTMLOptionElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'option' });
    }
  }
  ...
  static getSelector() {
    return `option[is="${this.getName()}"]`;
  }

  props: Partial<OptionProps<T>>;

  constructor(props?: Partial<OptionProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<OptionProps>} Partial props.
   */
  getDefaultProps(): Partial<OptionProps<T>> {
    return {};
  }
...
// Shorthand alias
export { OptionElement as Option };
