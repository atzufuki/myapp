...
export type Heading4Props<T = unknown> = IncomingProps<HTMLHeadingElement, T>;

export class Heading4Element<T = unknown> extends HTMLHeadingElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'h4' });
    }
  }
  ...
  static getSelector() {
    return `h4[is="${this.getName()}"]`;
  }

  props: Partial<Heading4Props<T>>;

  constructor(props?: Partial<Heading4Props<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<Heading4Props>} Partial props.
   */
  getDefaultProps(): Partial<Heading4Props<T>> {
    return {};
  }
...
// Shorthand alias
export { Heading4Element as Heading4 };
