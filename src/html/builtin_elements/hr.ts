...
export type HRProps<T = unknown> = IncomingProps<HTMLHRElement, T>;

export class HRElement<T = unknown> extends HTMLHRElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'hr' });
    }
  }
  ...
  static getSelector() {
    return `hr[is="${this.getName()}"]`;
  }

  props: Partial<HRProps<T>>;

  constructor(props?: Partial<HRProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<HRProps>} Partial props.
   */
  getDefaultProps(): Partial<HRProps<T>> {
    return {};
  }
