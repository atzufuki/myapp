...
export type ImageProps<T = unknown> = IncomingProps<HTMLImageElement, T>;

export class ImageElement<T = unknown> extends HTMLImageElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'img' });
    }
  }
  ...
  static getSelector() {
    return `img[is="${this.getName()}"]`;
  }

  props: Partial<ImageProps<T>>;

  constructor(props?: Partial<ImageProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<ImageProps>} Partial props.
   */
  getDefaultProps(): Partial<ImageProps<T>> {
    return {};
  }
