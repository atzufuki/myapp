...
export type TimeProps<T = unknown> = IncomingProps<HTMLTimeElement, T>;

export class TimeElement<T = unknown> extends HTMLTimeElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'time' });
    }
  }
  ...
  static getSelector() {
    return `time[is="${this.getName()}"]`;
  }

  props: Partial<TimeProps<T>>;

  constructor(props?: Partial<TimeProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TimeProps>} Partial props.
   */
  getDefaultProps(): Partial<TimeProps<T>> {
    return {};
  }
