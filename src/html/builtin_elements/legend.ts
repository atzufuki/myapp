...
export type LegendProps<T = unknown> = IncomingProps<HTMLLegendElement, T>;

export class LegendElement<T = unknown> extends HTMLLegendElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'legend' });
    }
  }
  ...
  static getSelector() {
    return `legend[is="${this.getName()}"]`;
  }

  props: Partial<LegendProps<T>>;

  constructor(props?: Partial<LegendProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<LegendProps>} Partial props.
   */
  getDefaultProps(): Partial<LegendProps<T>> {
    return {};
  }
...
// Shorthand alias
export { LegendElement as Legend };
