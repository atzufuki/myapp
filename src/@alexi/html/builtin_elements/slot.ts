...
export type SlotProps<T = unknown> = IncomingProps<HTMLSlotElement, T>;

export class SlotElement<T = unknown> extends HTMLSlotElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'slot' });
    }
  }
  ...
  static getSelector() {
    return `slot[is="${this.getName()}"]`;
  }

  props: Partial<SlotProps<T>>;

  constructor(props?: Partial<SlotProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<SlotProps>} Partial props.
   */
  getDefaultProps(): Partial<SlotProps<T>> {
    return {};
  }
