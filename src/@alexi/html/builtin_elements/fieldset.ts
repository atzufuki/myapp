...
export type FieldSetProps<T = unknown> = IncomingProps<HTMLFieldSetElement, T>;

export class FieldSetElement<T = unknown> extends HTMLFieldSetElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'fieldset' });
    }
  }
  ...
  static getSelector() {
    return `fieldset[is="${this.getName()}"]`;
  }

  props: Partial<FieldSetProps<T>>;

  constructor(props?: Partial<FieldSetProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<FieldSetProps>} Partial props.
   */
  getDefaultProps(): Partial<FieldSetProps<T>> {
    return {};
  }
...
// Shorthand alias
export { FieldSetElement as FieldSet };
