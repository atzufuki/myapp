...
export type DataListProps<T = unknown> = IncomingProps<HTMLDataListElement, T>;

export class DataListElement<T = unknown> extends HTMLDataListElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'datalist' });
    }
  }
  ...
  static getSelector() {
    return `datalist[is="${this.getName()}"]`;
  }

  props: Partial<DataListProps<T>>;

  constructor(props?: Partial<DataListProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<DataListProps>} Partial props.
   */
  getDefaultProps(): Partial<DataListProps<T>> {
    return {};
  }
...
// Shorthand alias
export { DataListElement as DataList };
