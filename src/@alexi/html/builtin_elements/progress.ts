...
export type ProgressProps<T = unknown> = IncomingProps<HTMLProgressElement, T>;

export class ProgressElement<T = unknown> extends HTMLProgressElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'progress' });
    }
  }
  ...
  static getSelector() {
    return `progress[is="${this.getName()}"]`;
  }

  props: Partial<ProgressProps<T>>;

  constructor(props?: Partial<ProgressProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<ProgressProps>} Partial props.
   */
  getDefaultProps(): Partial<ProgressProps<T>> {
    return {};
  }
...
// Shorthand alias
export { ProgressElement as Progress };
