...
export type DListProps<T = unknown> = IncomingProps<HTMLDListElement, T>;

export class DListElement<T = unknown> extends HTMLDListElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'dl' });
    }
  }
  ...
  static getSelector() {
    return `dl[is="${this.getName()}"]`;
  }

  props: Partial<DListProps<T>>;

  constructor(props?: Partial<DListProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<DListProps>} Partial props.
   */
  getDefaultProps(): Partial<DListProps<T>> {
    return {};
  }
