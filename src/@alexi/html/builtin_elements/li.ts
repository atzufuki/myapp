...
export type LIProps<T = unknown> = IncomingProps<HTMLLIElement, T>;

export class LIElement<T = unknown> extends HTMLLIElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'li' });
    }
  }
  ...
  static getSelector() {
    return `li[is="${this.getName()}"]`;
  }

  props: Partial<LIProps<T>>;

  constructor(props?: Partial<LIProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<LIProps>} Partial props.
   */
  getDefaultProps(): Partial<LIProps<T>> {
    return {};
  }
