...
export type TableHeadProps<T = unknown> = IncomingProps<
  HTMLTableCellElement,
  T
>;

export class TableHeadElement<T = unknown> extends HTMLTableCellElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'th' });
    }
  }
  ...
  static getSelector() {
    return `th[is="${this.getName()}"]`;
  }

  props: Partial<TableHeadProps<T>>;

  constructor(props?: Partial<TableHeadProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TableHeadProps>} Partial props.
   */
  getDefaultProps(): Partial<TableHeadProps<T>> {
    return {};
  }
...
// Shorthand alias
export { TableHeadElement as TableHead };
