...
export type AreaProps<T = unknown> = IncomingProps<HTMLAreaElement, T>;

export class AreaElement<T = unknown> extends HTMLAreaElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'area' });
    }
  }
  ...
  static getSelector() {
    return `area[is="${this.getName()}"]`;
  }

  props: Partial<AreaProps<T>>;

  constructor(props?: Partial<AreaProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<AreaProps>} Partial props.
   */
  getDefaultProps(): Partial<AreaProps<T>> {
    return {};
  }
