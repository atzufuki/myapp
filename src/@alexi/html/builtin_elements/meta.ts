...
export type MetaProps<T = unknown> = IncomingProps<HTMLMetaElement, T>;

export class MetaElement<T = unknown> extends HTMLMetaElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'meta' });
    }
  }
  ...
  static getSelector() {
    return `meta[is="${this.getName()}"]`;
  }

  props: Partial<MetaProps<T>>;

  constructor(props?: Partial<MetaProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<MetaProps>} Partial props.
   */
  getDefaultProps(): Partial<MetaProps<T>> {
    return {};
  }
