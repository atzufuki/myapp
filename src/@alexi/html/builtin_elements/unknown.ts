...
export type UnknownProps<T = unknown> = IncomingProps<HTMLUnknownElement, T>;

export class UnknownElement<T = unknown> extends HTMLUnknownElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'unknown' });
    }
  }
  ...
  static getSelector() {
    return `unknown[is="${this.getName()}"]`;
  }

  props: Partial<UnknownProps<T>>;

  constructor(props?: Partial<UnknownProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<UnknownProps>} Partial props.
   */
  getDefaultProps(): Partial<UnknownProps<T>> {
    return {};
  }
...
// Shorthand alias
export { UnknownElement as Unknown };
