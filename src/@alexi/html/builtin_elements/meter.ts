...
export type MeterProps<T = unknown> = IncomingProps<HTMLMeterElement, T>;

export class MeterElement<T = unknown> extends HTMLMeterElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'meter' });
    }
  }
  ...
  static getSelector() {
    return `meter[is="${this.getName()}"]`;
  }

  props: Partial<MeterProps<T>>;

  constructor(props?: Partial<MeterProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<MeterProps>} Partial props.
   */
  getDefaultProps(): Partial<MeterProps<T>> {
    return {};
  }
