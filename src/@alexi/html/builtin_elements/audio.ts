...
export type AudioProps<T = unknown> = IncomingProps<HTMLAudioElement, T>;

export class AudioElement<T = unknown> extends HTMLAudioElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'audio' });
    }
  }
  ...
  static getSelector() {
    return `audio[is="${this.getName()}"]`;
  }

  props: Partial<AudioProps<T>>;

  constructor(props?: Partial<AudioProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<AudioProps>} Partial props.
   */
  getDefaultProps(): Partial<AudioProps<T>> {
    return {};
  }
