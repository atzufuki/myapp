...
export type MapProps<T = unknown> = IncomingProps<HTMLMapElement, T>;

export class MapElement<T = unknown> extends HTMLMapElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'map' });
    }
  }
  ...
  static getSelector() {
    return `map[is="${this.getName()}"]`;
  }

  props: Partial<MapProps<T>>;

  constructor(props?: Partial<MapProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<MapProps>} Partial props.
   */
  getDefaultProps(): Partial<MapProps<T>> {
    return {};
  }
