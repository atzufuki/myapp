...
export type TableCaptionProps<T = unknown> = IncomingProps<
  HTMLTableCaptionElement,
  T
>;

export class TableCaptionElement<T = unknown> extends HTMLTableCaptionElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'caption' });
    }
  }
  ...
  static getSelector() {
    return `caption[is="${this.getName()}"]`;
  }

  props: Partial<TableCaptionProps<T>>;

  constructor(props?: Partial<TableCaptionProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TableCaptionProps>} Partial props.
   */
  getDefaultProps(): Partial<TableCaptionProps<T>> {
    return {};
  }
...
// Shorthand alias
export { TableCaptionElement as TableCaption };
