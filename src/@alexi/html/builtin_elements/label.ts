...
export type LabelProps<T = unknown> = IncomingProps<HTMLLabelElement, T>;

export class LabelElement<T = unknown> extends HTMLLabelElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'label' });
    }
  }
  ...
  static getSelector() {
    return `label[is="${this.getName()}"]`;
  }

  props: Partial<LabelProps<T>>;

  constructor(props?: Partial<LabelProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<LabelProps>} Partial props.
   */
  getDefaultProps(): Partial<LabelProps<T>> {
    return {};
  }
