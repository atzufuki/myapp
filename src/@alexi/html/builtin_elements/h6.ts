...
export type Heading6Props<T = unknown> = IncomingProps<HTMLHeadingElement, T>;

export class Heading6Element<T = unknown> extends HTMLHeadingElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'h6' });
    }
  }
  ...
  static getSelector() {
    return `h6[is="${this.getName()}"]`;
  }

  props: Partial<Heading6Props<T>>;

  constructor(props?: Partial<Heading6Props<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<Heading6Props>} Partial props.
   */
  getDefaultProps(): Partial<Heading6Props<T>> {
    return {};
  }
...
// Shorthand alias
export { Heading6Element as Heading6 };
