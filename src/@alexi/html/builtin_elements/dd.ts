...
export type DDetailsProps<T = unknown> = IncomingProps<HTMLElement, T>;

export class DDetailsElement<T = unknown> extends HTMLElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'dd' });
    }
  }
  ...
  static getSelector() {
    return `dd[is="${this.getName()}"]`;
  }

  props: Partial<DDetailsProps<T>>;

  constructor(props?: Partial<DDetailsProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<DDetailsProps>} Partial props.
   */
  getDefaultProps(): Partial<DDetailsProps<T>> {
    return {};
  }
...
// Shorthand alias
export { DDetailsElement as DDetails };
