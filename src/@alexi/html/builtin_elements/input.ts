...
export type InputProps<T = unknown> = IncomingProps<HTMLInputElement, T>;

export class InputElement<T = unknown> extends HTMLInputElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'input' });
    }
  }
  ...
  static getSelector() {
    return `input[is="${this.getName()}"]`;
  }

  props: Partial<InputProps<T>>;

  constructor(props?: Partial<InputProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<InputProps>} Partial props.
   */
  getDefaultProps(): Partial<InputProps<T>> {
    return {};
  }
