...
export type PictureProps<T = unknown> = IncomingProps<HTMLPictureElement, T>;

export class PictureElement<T = unknown> extends HTMLPictureElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'picture' });
    }
  }
  ...
  static getSelector() {
    return `picture[is="${this.getName()}"]`;
  }

  props: Partial<PictureProps<T>>;

  constructor(props?: Partial<PictureProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<PictureProps>} Partial props.
   */
  getDefaultProps(): Partial<PictureProps<T>> {
    return {};
  }
...
// Shorthand alias
export { PictureElement as Picture };
