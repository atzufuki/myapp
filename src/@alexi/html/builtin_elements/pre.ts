...
export type PreProps<T = unknown> = IncomingProps<HTMLPreElement, T>;

export class PreElement<T = unknown> extends HTMLPreElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'pre' });
    }
  }
  ...
  static getSelector() {
    return `pre[is="${this.getName()}"]`;
  }

  props: Partial<PreProps<T>>;

  constructor(props?: Partial<PreProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<PreProps>} Partial props.
   */
  getDefaultProps(): Partial<PreProps<T>> {
    return {};
  }
