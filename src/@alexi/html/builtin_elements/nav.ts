...
export type NavProps<T = unknown> = IncomingProps<HTMLElement, T>;

export class NavElement<T = unknown> extends HTMLElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'nav' });
    }
  }
  ...
  static getSelector() {
    return `nav[is="${this.getName()}"]`;
  }

  props: Partial<NavProps<T>>;

  constructor(props?: Partial<NavProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<NavProps>} Partial props.
   */
  getDefaultProps(): Partial<NavProps<T>> {
    return {};
  }
