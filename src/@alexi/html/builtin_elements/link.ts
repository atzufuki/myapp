...
export type LinkProps<T = unknown> = IncomingProps<HTMLLinkElement, T>;

export class LinkElement<T = unknown> extends HTMLLinkElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'link' });
    }
  }
  ...
  static getSelector() {
    return `link[is="${this.getName()}"]`;
  }

  props: Partial<LinkProps<T>>;

  constructor(props?: Partial<LinkProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<LinkProps>} Partial props.
   */
  getDefaultProps(): Partial<LinkProps<T>> {
    return {};
  }
