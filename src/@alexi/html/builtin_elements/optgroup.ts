...
export type OptGroupProps<T = unknown> = IncomingProps<HTMLOptGroupElement, T>;

export class OptGroupElement<T = unknown> extends HTMLOptGroupElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'optgroup' });
    }
  }
  ...
  static getSelector() {
    return `optgroup[is="${this.getName()}"]`;
  }

  props: Partial<OptGroupProps<T>>;

  constructor(props?: Partial<OptGroupProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<OptGroupProps>} Partial props.
   */
  getDefaultProps(): Partial<OptGroupProps<T>> {
    return {};
  }
...
// Shorthand alias
export { OptGroupElement as OptGroup };
