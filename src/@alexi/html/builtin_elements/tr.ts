...
export type TableRowProps<T = unknown> = IncomingProps<HTMLTableRowElement, T>;

export class TableRowElement<T = unknown> extends HTMLTableRowElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'tr' });
    }
  }
  ...
  static getSelector() {
    return `tr[is="${this.getName()}"]`;
  }

  props: Partial<TableRowProps<T>>;

  constructor(props?: Partial<TableRowProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TableRowProps>} Partial props.
   */
  getDefaultProps(): Partial<TableRowProps<T>> {
    return {};
  }
...
// Shorthand alias
export { TableRowElement as TableRow };
