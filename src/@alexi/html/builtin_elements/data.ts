...
export type DataProps<T = unknown> = IncomingProps<HTMLDataElement, T>;

export class DataElement<T = unknown> extends HTMLDataElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'data' });
    }
  }
  ...
  static getSelector() {
    return `data[is="${this.getName()}"]`;
  }

  props: Partial<DataProps<T>>;

  constructor(props?: Partial<DataProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<DataProps>} Partial props.
   */
  getDefaultProps(): Partial<DataProps<T>> {
    return {};
  }
