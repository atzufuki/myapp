...
export type BaseProps<T = unknown> = IncomingProps<HTMLBaseElement, T>;

export class BaseElement<T = unknown> extends HTMLBaseElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'base' });
    }
  }
  ...
  static getSelector() {
    return `base[is="${this.getName()}"]`;
  }

  props: Partial<BaseProps<T>>;

  constructor(props?: Partial<BaseProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<BaseProps>} Partial props.
   */
  getDefaultProps(): Partial<BaseProps<T>> {
    return {};
  }
