...
export type MediaProps<T = unknown> = IncomingProps<HTMLMediaElement, T>;

export class MediaElement<T = unknown> extends HTMLMediaElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'media' });
    }
  }
  ...
  static getSelector() {
    return `media[is="${this.getName()}"]`;
  }

  props: Partial<MediaProps<T>>;

  constructor(props?: Partial<MediaProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<MediaProps>} Partial props.
   */
  getDefaultProps(): Partial<MediaProps<T>> {
    return {};
  }
