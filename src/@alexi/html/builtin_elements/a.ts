...
export type AnchorProps<T = unknown> = IncomingProps<HTMLAnchorElement, T>;

export class AnchorElement<T = unknown> extends HTMLAnchorElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'a' });
    }
  }
  ...
  static getSelector() {
    return `a[is="${this.getName()}"]`;
  }

  props: Partial<AnchorProps<T>>;

  constructor(props?: Partial<AnchorProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<AnchorProps>} Partial props.
   */
  getDefaultProps(): Partial<AnchorProps<T>> {
    return {};
  }
...
// Shorthand alias
export { AnchorElement as Anchor };
