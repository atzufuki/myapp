...
export type OutputProps<T = unknown> = IncomingProps<HTMLOutputElement, T>;

export class OutputElement<T = unknown> extends HTMLOutputElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'output' });
    }
  }
  ...
  static getSelector() {
    return `output[is="${this.getName()}"]`;
  }

  props: Partial<OutputProps<T>>;

  constructor(props?: Partial<OutputProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<OutputProps>} Partial props.
   */
  getDefaultProps(): Partial<OutputProps<T>> {
    return {};
  }
...
// Shorthand alias
export { OutputElement as Output };
