...
export type VideoProps<T = unknown> = IncomingProps<HTMLVideoElement, T>;

export class VideoElement<T = unknown> extends HTMLVideoElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'video' });
    }
  }
  ...
  static getSelector() {
    return `video[is="${this.getName()}"]`;
  }

  props: Partial<VideoProps<T>>;

  constructor(props?: Partial<VideoProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<VideoProps>} Partial props.
   */
  getDefaultProps(): Partial<VideoProps<T>> {
    return {};
  }
