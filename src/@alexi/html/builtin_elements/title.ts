...
export type TitleProps<T = unknown> = IncomingProps<HTMLTitleElement, T>;

export class TitleElement<T = unknown> extends HTMLTitleElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'title' });
    }
  }
  ...
  static getSelector() {
    return `title[is="${this.getName()}"]`;
  }

  props: Partial<TitleProps<T>>;

  constructor(props?: Partial<TitleProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TitleProps>} Partial props.
   */
  getDefaultProps(): Partial<TitleProps<T>> {
    return {};
  }
