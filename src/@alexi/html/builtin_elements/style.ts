...
export type StyleProps<T = unknown> = IncomingProps<HTMLStyleElement, T>;

export class StyleElement<T = unknown> extends HTMLStyleElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'style' });
    }
  }
  ...
  static getSelector() {
    return `style[is="${this.getName()}"]`;
  }

  props: Partial<StyleProps<T>>;

  constructor(props?: Partial<StyleProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<StyleProps>} Partial props.
   */
  getDefaultProps(): Partial<StyleProps<T>> {
    return {};
  }
