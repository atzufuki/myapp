...
export type SourceProps<T = unknown> = IncomingProps<HTMLSourceElement, T>;

export class SourceElement<T = unknown> extends HTMLSourceElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'src' });
    }
  }
  ...
  static getSelector() {
    return `src[is="${this.getName()}"]`;
  }

  props: Partial<SourceProps<T>>;

  constructor(props?: Partial<SourceProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<SourceProps>} Partial props.
   */
  getDefaultProps(): Partial<SourceProps<T>> {
    return {};
  }
...
// Shorthand alias
export { SourceElement as Source };
