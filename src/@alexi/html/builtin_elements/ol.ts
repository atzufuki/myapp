...
export type OListProps<T = unknown> = IncomingProps<HTMLOListElement, T>;

export class OListElement<T = unknown> extends HTMLOListElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'ol' });
    }
  }
  ...
  static getSelector() {
    return `ol[is="${this.getName()}"]`;
  }

  props: Partial<OListProps<T>>;

  constructor(props?: Partial<OListProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<OListProps>} Partial props.
   */
  getDefaultProps(): Partial<OListProps<T>> {
    return {};
  }
