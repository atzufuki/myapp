...
export type SelectProps<T = unknown> = IncomingProps<HTMLSelectElement, T>;

export class SelectElement<T = unknown> extends HTMLSelectElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'select' });
    }
  }
  ...
  static getSelector() {
    return `select[is="${this.getName()}"]`;
  }

  props: Partial<SelectProps<T>>;

  constructor(props?: Partial<SelectProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<SelectProps>} Partial props.
   */
  getDefaultProps(): Partial<SelectProps<T>> {
    return {};
  }
