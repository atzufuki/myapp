...
export type TableCellProps<T = unknown> = IncomingProps<
  HTMLTableCellElement,
  T
>;

export class TableCellElement<T = unknown> extends HTMLTableCellElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'td' });
    }
  }
  ...
  static getSelector() {
    return `td[is="${this.getName()}"]`;
  }

  props: Partial<TableCellProps<T>>;

  constructor(props?: Partial<TableCellProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TableCellProps>} Partial props.
   */
  getDefaultProps(): Partial<TableCellProps<T>> {
    return {};
  }
...
// Shorthand alias
export { TableCellElement as TableCell };
