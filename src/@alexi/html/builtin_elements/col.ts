...
export type TableColProps<T = unknown> = IncomingProps<HTMLTableColElement, T>;

export class TableColElement<T = unknown> extends HTMLTableColElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'col' });
    }
  }
  ...
  static getSelector() {
    return `col[is="${this.getName()}"]`;
  }

  props: Partial<TableColProps<T>>;

  constructor(props?: Partial<TableColProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TableColProps>} Partial props.
   */
  getDefaultProps(): Partial<TableColProps<T>> {
    return {};
  }
...
// Shorthand alias
export { TableColElement as TableCol };
