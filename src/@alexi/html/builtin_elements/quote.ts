...
export type QuoteProps<T = unknown> = IncomingProps<HTMLQuoteElement, T>;

export class QuoteElement<T = unknown> extends HTMLQuoteElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'quote' });
    }
  }
  ...
  static getSelector() {
    return `quote[is="${this.getName()}"]`;
  }

  props: Partial<QuoteProps<T>>;

  constructor(props?: Partial<QuoteProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<QuoteProps>} Partial props.
   */
  getDefaultProps(): Partial<QuoteProps<T>> {
    return {};
  }
