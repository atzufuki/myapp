...
export type Heading5Props<T = unknown> = IncomingProps<HTMLHeadingElement, T>;

export class Heading5Element<T = unknown> extends HTMLHeadingElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'h5' });
    }
  }
  ...
  static getSelector() {
    return `h5[is="${this.getName()}"]`;
  }

  props: Partial<Heading5Props<T>>;

  constructor(props?: Partial<Heading5Props<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<Heading5Props>} Partial props.
   */
  getDefaultProps(): Partial<Heading5Props<T>> {
    return {};
  }
...
// Shorthand alias
export { Heading5Element as Heading5 };
