...
export type UListProps<T = unknown> = IncomingProps<HTMLUListElement, T>;

export class UListElement<T = unknown> extends HTMLUListElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'ul' });
    }
  }
  ...
  static getSelector() {
    return `ul[is="${this.getName()}"]`;
  }

  props: Partial<UListProps<T>>;

  constructor(props?: Partial<UListProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<UListProps>} Partial props.
   */
  getDefaultProps(): Partial<UListProps<T>> {
    return {};
  }
