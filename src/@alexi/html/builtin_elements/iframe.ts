...
export type IFrameProps<T = unknown> = IncomingProps<HTMLIFrameElement, T>;

export class IFrameElement<T = unknown> extends HTMLIFrameElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'iframe' });
    }
  }
  ...
  static getSelector() {
    return `iframe[is="${this.getName()}"]`;
  }

  props: Partial<IFrameProps<T>>;

  constructor(props?: Partial<IFrameProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<IFrameProps>} Partial props.
   */
  getDefaultProps(): Partial<IFrameProps<T>> {
    return {};
  }
...
// Shorthand alias
export { IFrameElement as IFrame };
