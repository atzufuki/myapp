...
export type CanvasProps<T = unknown> = IncomingProps<HTMLCanvasElement, T>;

export class CanvasElement<T = unknown> extends HTMLCanvasElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'canvas' });
    }
  }
  ...
  static getSelector() {
    return `canvas[is="${this.getName()}"]`;
  }

  props: Partial<CanvasProps<T>>;

  constructor(props?: Partial<CanvasProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<CanvasProps>} Partial props.
   */
  getDefaultProps(): Partial<CanvasProps<T>> {
    return {};
  }
