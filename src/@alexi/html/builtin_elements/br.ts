...
export type BRProps<T = unknown> = IncomingProps<HTMLBRElement, T>;

export class BRElement<T = unknown> extends HTMLBRElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'br' });
    }
  }
  ...
  static getSelector() {
    return `br[is="${this.getName()}"]`;
  }

  props: Partial<BRProps<T>>;

  constructor(props?: Partial<BRProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<BRProps>} Partial props.
   */
  getDefaultProps(): Partial<BRProps<T>> {
    return {};
  }
