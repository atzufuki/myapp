...
export type ObjectProps<T = unknown> = IncomingProps<HTMLObjectElement, T>;

export class ObjectElement<T = unknown> extends HTMLObjectElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'object' });
    }
  }
  ...
  static getSelector() {
    return `object[is="${this.getName()}"]`;
  }

  props: Partial<ObjectProps<T>>;

  constructor(props?: Partial<ObjectProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<ObjectProps>} Partial props.
   */
  getDefaultProps(): Partial<ObjectProps<T>> {
    return {};
  }
...
// Shorthand alias
export { ObjectElement as Object };
