...
export type ModProps<T = unknown> = IncomingProps<HTMLModElement, T>;

export class ModElement<T = unknown> extends HTMLModElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'mod' });
    }
  }
  ...
  static getSelector() {
    return `mod[is="${this.getName()}"]`;
  }

  props: Partial<ModProps<T>>;

  constructor(props?: Partial<ModProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<ModProps>} Partial props.
   */
  getDefaultProps(): Partial<ModProps<T>> {
    return {};
  }
