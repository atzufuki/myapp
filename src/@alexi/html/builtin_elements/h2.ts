...
export type Heading2Props<T = unknown> = IncomingProps<HTMLHeadingElement, T>;

export class Heading2Element<T = unknown> extends HTMLHeadingElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'h2' });
    }
  }
  ...
  static getSelector() {
    return `h2[is="${this.getName()}"]`;
  }

  props: Partial<Heading2Props<T>>;

  constructor(props?: Partial<Heading2Props<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<Heading2Props>} Partial props.
   */
  getDefaultProps(): Partial<Heading2Props<T>> {
    return {};
  }
...
// Shorthand alias
export { Heading2Element as Heading2 };
