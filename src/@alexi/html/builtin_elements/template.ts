...
export type TemplateProps<T = unknown> = IncomingProps<HTMLTemplateElement, T>;

export class TemplateElement<T = unknown> extends HTMLTemplateElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'template' });
    }
  }
  ...
  static getSelector() {
    return `template[is="${this.getName()}"]`;
  }

  props: Partial<TemplateProps<T>>;

  constructor(props?: Partial<TemplateProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TemplateProps>} Partial props.
   */
  getDefaultProps(): Partial<TemplateProps<T>> {
    return {};
  }
...
// Shorthand alias
export { TemplateElement as Template };
