...
export type FrameSetProps<T = unknown> = IncomingProps<HTMLFrameSetElement, T>;

export class FrameSetElement<T = unknown> extends HTMLFrameSetElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'frameset' });
    }
  }
  ...
  static getSelector() {
    return `frameset[is="${this.getName()}"]`;
  }

  props: Partial<FrameSetProps<T>>;

  constructor(props?: Partial<FrameSetProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<FrameSetProps>} Partial props.
   */
  getDefaultProps(): Partial<FrameSetProps<T>> {
    return {};
  }
...
// Shorthand alias
export { FrameSetElement as FrameSet };
