...
export type TableSectionProps<T = unknown> = IncomingProps<
  HTMLTableSectionElement,
  T
>;

export class TableSectionElement<T = unknown> extends HTMLTableSectionElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'tbody' });
    }
  }
  ...
  static getSelector() {
    return `tbody[is="${this.getName()}"]`;
  }

  props: Partial<TableSectionProps<T>>;

  constructor(props?: Partial<TableSectionProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TableSectionProps>} Partial props.
   */
  getDefaultProps(): Partial<TableSectionProps<T>> {
    return {};
  }
...
// Shorthand alias
export { TableSectionElement as TableSection };
