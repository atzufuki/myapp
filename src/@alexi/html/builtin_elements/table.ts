...
export type TableProps<T = unknown> = IncomingProps<HTMLTableElement, T>;

export class TableElement<T = unknown> extends HTMLTableElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'table' });
    }
  }
  ...
  static getSelector() {
    return `table[is="${this.getName()}"]`;
  }

  props: Partial<TableProps<T>>;

  constructor(props?: Partial<TableProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<TableProps>} Partial props.
   */
  getDefaultProps(): Partial<TableProps<T>> {
    return {};
  }
