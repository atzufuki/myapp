...
export type ParagraphProps<T = unknown> = IncomingProps<
  HTMLParagraphElement,
  T
>;

export class ParagraphElement<T = unknown> extends HTMLParagraphElement {
  ...
  static define(name: string) {
    ...
    if (!defined) {
      customElements.define(name, this, { extends: 'p' });
    }
  }
  ...
  static getSelector() {
    return `p[is="${this.getName()}"]`;
  }

  props: Partial<ParagraphProps<T>>;

  constructor(props?: Partial<ParagraphProps<T>>) {
    super();
    this.props = props ?? {};
  }
   ...
   * @returns {Partial<ParagraphProps>} Partial props.
   */
  getDefaultProps(): Partial<ParagraphProps<T>> {
    return {};
  }
...
// Shorthand alias
export { ParagraphElement as Paragraph };
